...
Below, we show how to fit a meta-analysis using the `equalto` covariance structure in `glmmTMB` R package, which enables  to specify the sampling error variance-covariance (VCV) matrix.
...
## Changing optimization method in `glmmTMB`
...
knitr::kable(pkg_versions, align = c("l", "l"))
```
