...
```{r}
est <- exp(coef(summary(nma_cs_fit))$cond[,1])
ci_prof <- exp(confint(nma_cs_fit, parm="beta_", method = "profile"))

results_nma <- data.frame(
  Estimate = est,
  Lower_CI = ci_prof[, 1],
  Upper_CI = ci_prof[, 2]
)
...
Important note: currently `glmmTMB` doesn't support small sample adjustments (though it may be added in future developments).
...
## As of July 2026: you need to use the following branch
#detach("package:orchaRd", unload = TRUE)
#remove.packages("orchaRd")
#devtools::install_github("daniel1noble/orchaRd", ref = "glmmtmb-rma-adapter", force = TRUE)
