...
### Filter out all sim repetitions if one model has a convergence warning or didn't compute
...
#### Summaries for latex tables of convergence 
# All sim results
...
################ 5. Plots - all results agreements ###############
...
# mu SE dataset - package agreement
mu_se_hat <- res_all |> 
  filter(model %in% c("rma.uni", "glmmTMB_RE")) |> 
  group_by(measure, model) |> 
  mutate(.idx = row_number(),
...
ggsave(filename = "results/figures/Figure_NormalNormal_all.pdf", width = 11, height = 9)
...
ggsave(filename = "results/figures/Figure_binom_all.pdf", width = 8, height = 10)
...
ggsave(filename = "results/figures/Figure_poisson_all.pdf", width = 5, height = 10)
...
################ 6. Plots - converged results agreements ###############

#### NORMAL-NORMAL
# mu estimate dataset - package agreement
mu_hat <- res |> 
  filter(model %in% c("rma.uni", "glmmTMB_RE")) |> 
  group_by(measure, model) |> 
  mutate(.idx = row_number(),
...
plot_mu <- ggplot(mu_hat, aes(x = `rma.uni`, y = glmmTMB_RE, colour = measure)) +
  ...
  theme_bw() +
  theme(legend.position = "none")
#ggsave(filename = "results/figures/plot_agreement_mu.png", width = 11, height = 3)
...
plot_mu_se <- ggplot(mu_se_hat, aes(x = `rma.uni`, y = glmmTMB_RE, colour = measure)) +
  ...
  theme_bw() +
  theme(legend.position = "none")
#ggsave(filename = "results/figures/plot_agreement_SE.png", width = 11, height = 3)
...
plot_tau2 <- ggplot(tau2_hat, aes(x = `rma.uni`, y = glmmTMB_RE, colour = measure)) +
  ...
  theme_bw() +
  theme(legend.position = "none")
#ggsave(filename = "results/figures/plot_agreement_tau2.png", width = 11, height = 3)
